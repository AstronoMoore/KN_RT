...
for filename in csv_files:
    ...
    data['Fixed arrival time'] = data.apply(lambda row: row['Arrival time'] + (
        row['Day'] * 24 * 60 * 60) + (np.random.random()-0.5)*timestep*3600, axis=1)
    dfs.append(data)
...
'''
df = []
df = pd.DataFrame({'Arrival time': [], 'Fixed arrival time': [],
                  'Wavelength': [], 'BB Flux': [], 'Day': []})
...
def frac_df(df, low_bound, high_bound):
    ...
    fraction_dataframe = df[(df['Fraction time'] > low_bound) & (
        df['Fraction time'] <= high_bound)]
...
def day_df(df, low_time, high_time):
    ...
    time_dataframe = df[(df['Fixed arrival time'] > low_time)
                        & (df['Fixed arrival time'] <= high_time)]
...
def generate_arrays(hour_dataframe):
    ...
    for i in range(0, len(hour_wavelength)):
        bb_weight_hour[i] = (bb_weight_hour[i] * float(nbins) /
                             npacks) * hour_bb[i] * ((0.1 * release_day[i]) ** 2)
